...
/**
 * Fetch the user summary (balance, monthly usage, wallet info).
 *
 * GET https://platform.deepseek.com/api/v0/users/get_user_summary
 * Auth: Bearer <token>
 *
 * Returns the biz_data payload:
 *   { current_token, monthly_usage, normal_wallets, bonus_wallets,
 *     monthly_costs, monthly_token_usage, ... }
 */
export async function fetchUserSummary(token) {
  ...
  if (json.code !== 0) {
    throw new Error(`DeepSeek summary API error: ${json.msg || json.code}`);
  }
