...
export function aggregateByDay(rows, rate, currency = "USD") {
  ...
  for (const row of rows) {
    ...
    switch (type) {
      ...
      case "input_cache_miss_tokens":
        ...
        break;
      case "_total_cost":
        g.bill_amount_usd += amt;
        break;
    }

    // Accumulate USD bill only when a price is present (used by
    // legacy CSV rows that may still be cached).
    if (price && price !== "") {
  ...
  for (const g of groups.values()) {
    ...
    result[g.date].push({
      ...
      bill_amount: rate ? Math.round(g.bill_amount_usd * rate * 100) / 100 : 0,
    });
  }
